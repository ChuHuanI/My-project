...
class Calculator(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # First row
        tk.Button(btns_frame, text="C", fg=btn_fg, bg=clear_bg, cursor="hand2", font=btn_font, command=self.clear_input).grid(row=0, column=0, sticky="nsew", padx=1, pady=1)
        tk.Button(btns_frame, text="<-", fg=btn_fg, bg=clear_bg, cursor="hand2", font=btn_font, command=self.backspace).grid(row=0, column=1, sticky="nsew", padx=1, pady=1)
    ...
    def bind_keyboard(self):
        ...
        for key in "0123456789.+-*/":
            self.bind(key, lambda event, k=key: self.add_to_expression(k))
    ...
    def calculate(self):
        try:
            # Replace multiple operators with the last one, except for scientific notation
            # This is a simple fix and might not cover all edge cases
            import re
            calculation = re.sub(r'([+\-*/]){2,}', r'\1', self.expression)
            if calculation == "":
                return
